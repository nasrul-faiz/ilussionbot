...
async function downloadAudio(url, { timeoutMs = 180000 } = {}) {
    ...
    try {
        ...
        if (canExtract) {
            try {
                await runYtDlp(extractArgs, timeoutMs);
            } catch (extractErr) {
                await runYtDlp(directAudioArgs, timeoutMs);
            }
        } else {
            await runYtDlp(directAudioArgs, timeoutMs);
        }
...
async function downloadVideo(url, { maxHeight = 720, timeoutMs = 300000 } = {}) {
    ...
    const dir = makeWorkDir();
    const out = path.join(dir, 'out.%(ext)s');
    const canMerge = hasFfmpeg();
    const mergedFmt = `bv*[height<=${maxHeight}][ext=mp4]+ba[ext=m4a]/b[height<=${maxHeight}][ext=mp4]/b[height<=${maxHeight}]/bv*+ba/b`;
    const progressiveFmt = `b[ext=mp4][height<=${maxHeight}]/b[height<=${maxHeight}][ext=mp4]/b[ext=mp4]/best[ext=mp4]/best`;
    try {
        if (canMerge) {
            try {
                await runYtDlp([
